...
**Lineage can be generated.** Where the paper uses hypothesis-namespaced macros, every `\Hxxxx…` in the source names its hypothesis, so `scripts/check-evidence-chain.py` emits the claim→hypothesis map by traversal rather than by anyone filling a column in. Prefer it: a generated lineage column cannot drift from the macros the paper actually cites, and a claim citing no macro is immediately visible as either a transcribed number (a Satellite-claim instance) or a claim with no measured basis. Hand-filled lineage remains acceptable where a paper predates the macro chain.
