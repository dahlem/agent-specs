...
- `shadow_register.md` (from `hypothesis-register-keeper`, `op: reconstruct`, optional) — the hypotheses the paper states or implies it tested, each marked for whether the paper evidences commitment *before* results; plus a prediction-claim audit, a file-drawer estimate, and a shape diagnostic over the set. Use it for three finding types only: prose asserting prediction the reported process cannot support (quoted), exploratory work framed as confirmatory, and conditions described but never reported. A paper that claims no predictions owes no register, and "did not pre-register" is not a finding you may issue — most venues do not require it, and a shadow register that comes back uninformative is an honest result, not a gap.
...
Ledger Mode also consumes `reconciliation.md` (`hypothesis-register-keeper`, `op: reconcile`) when the paper has a hypothesis register. It answers a question the ledger cannot: not whether each claim is dispositioned, but whether the paper reports the hypotheses the project actually tested. Its scope-drift entries are findings against specific sentences; its file-drawer entries are findings against the paper's completeness and belong in your Phase verdicts on analysis and reporting, not in the claim-by-claim sweep. A ledger that is fresh and a reconciliation that is clean are different guarantees, and the second is the one a replication would test.
