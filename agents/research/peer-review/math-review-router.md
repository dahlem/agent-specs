...
description: "Use this agent when the peer-review compression flags `theory_heavy: true`: it verifies the gate, routes per-theorem questions to the math-brainstorming agents (reframer, perturber, math-constructor, math-strategist, obstructor), and consolidates their outputs into one bundle for claim-interrogator. Conditional stage of the peer-review pipeline (between paper-compressor and claim-interrogator); it routes and synthesizes rather than duplicating math-brainstorming logic.\n\nExample:\n\n- User: \"This paper has theorems. How do we audit them?\"\n  Assistant: \"I'll use the math-review-router agent to dispatch per-theorem questions to the math-brainstorming agents and consolidate the results.\""
model: opus
color: yellow
---
