...
description: "Use this agent to run the research-shaping pipeline that turns a body of work into the one paper hiding inside it: sequences `research-divergence-cartographer` → `red-thread-selector` → optional `scientific-narrative-architect` Sculpt Mode → handoff to `06-argument-architect`. Supports `stage` (diverge | select | sculpt | argue) and `sculpt` (auto | on | off); handles `selection_outcome: deferred` as a first-class stop.\n\nExample:\n\n- User: \"I have months of accumulated work and need to decide which paper it should become.\"\n  Assistant: \"I'll use the research-shaping-orchestrator agent to run divergence, selection, and sculpting, then hand off to phase 06.\""
model: opus
color: green
---
