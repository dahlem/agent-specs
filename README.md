...
| Agent | Purpose |
|---|---|
| [`paper-compressor`](#paper-compressor) | Lossless-on-claims compression: tier inventory, methods, datasets, theorem + figure/table indexes, cutoff |
...
| Agent | Purpose |
|---|---|
| [`reframer`](#reframer) | 8–12 alternative problem encodings via seven transformation types, each with a discharged fidelity obligation |
| [`perturber`](#perturber) | 10–15 nearby variants by modifying assumptions, parameters, constraints |
| [`math-constructor`](#math-constructor) | 5–10 explicit objects/examples via seven construction strategies, plus discriminating instances |
...
**Headings carry findings, not process ordinals.** The title's discipline applies at every level: `Test 7: What the geometry forbids stays forbidden in a network` contains a good heading and a defect, and deleting `Test 7:` improves it with no loss. The diagnostic is to read the table of contents alone — if it lists what the authors *did* rather than what the field now *knows*, the headings are process labels.

**Research history is narration, never assertion.** The paper's spine states everything in its final correct form at first appearance; what fails is leading with something the authors know is wrong, letting it stand as load-bearing, and correcting it pages later — every skimming reader retains the wrong version. The test is *where the reader is when the correction arrives*: same breath ("one might expect X; in fact Y") is motivation and welcome; sections later is a defect. This does not conflict with the conclusion's digestion surface — history belongs where it informs, after the reader holds the correct picture, not where it misinforms. The project's own history is preserved in `hypothesis-register/` and `research-memory/` anyway, which is what frees the spine to state only what is true.
...
**The fidelity obligation.** A reframing is a translation, and every translation can silently change what is being said — the failure is not that a reframing is hard, but that it looks equivalent, is treated as equivalent, and is not. Each reframing must therefore carry an explicit **correspondence map** (which object, index, or constraint becomes which), a **fidelity classification** (`equivalent` | `sufficient-only` R ⟹ P | `necessary-only` P ⟹ R | `heuristic`), and a **witness**: one small instance evaluated under both formulations. The witness must be non-degenerate in every index the map touches — a square matrix agrees under a transposed convention and tests nothing. Asserting equivalence without discharging it is a forbidden behavior. This matters more here than in a formal setting, not less: Lean at least type-checks a mistranslation, whereas nothing at all checks a prose reframing.
...
An eighth category, **discriminating instances**, builds objects not to satisfy a constraint but to *separate two candidate readings* of one. Where a statement arrived through a translation — formalized from prose, carried across a reframing, transcribed from a paper — the plausible readings usually agree on most instances, and effort then goes into establishing something true that nobody meant. The discriminating instance is the cheapest available insurance: name the rival readings explicitly, break every symmetry the convention touches (unequal dimensions, distinct entries, no accidental symmetry, generic values), keep it small enough to check by hand, and evaluate every reading on it. A 2×3 matrix with six distinct entries discriminates orientation where a symmetric 2×2 of ones cannot. It supplies `obstructor`'s encoding attack and `reframer`'s fidelity witnesses, and a null result — the statement matches the intended reading — is reported too, since silence is indistinguishable from not having checked.
...
Stress-tests ideas, conjectures, and proof strategies by searching for counterexamples, hidden assumptions, and structural impossibilities. Applies nine adversarial strategies (counterexample construction, minimal obstruction analysis, hidden assumption detection, adversarial perturbation, boundary stress testing, invariant violation, logical gap detection, encoding attack, vacuity attack). Generates 5-8 adversarial instances, isolates root causes explaining *why* ideas fail, and suggests minimal repairs. Issues verdicts: Fatal, Wounded, Misspecified, Vacuous, or Robust.

**Attacking the encoding, not the claim.** Strategies 1–7 assume the statement in front of you is the statement someone meant. When a claim arrives through a translation, the encoding is an independent attack surface that fails silently — a mistranslated statement can be perfectly true, and proving it establishes nothing. The agent enumerates the plausible rival readings (`vec` by rows vs columns, index order `ij` vs `ji`, transpose placement, argument order, `<` vs `≤` at a boundary), constructs the smallest instance on which they disagree, and reports a `Misspecified` verdict when the target matches a rival rather than the intent. That verdict judges the *encoding*, not the idea, and routes back to whoever performed the translation. The **vacuity attack** runs first against the premises: exhibit one object satisfying every hypothesis jointly, or report its absence as the obstruction. This is the one attack where the degenerate instance is the finding rather than a cheap shot, and it is explicitly exempted from the usual prohibition on trivial edge cases. `Robust` may never be reported when the encoding was not audited or no non-vacuity witness was exhibited.
...
**The fidelity thread** runs across four of them: `reframer` discharges the fidelity obligation on every reframing it emits, `math-constructor` supplies the discriminating instances that make such a discharge checkable, `obstructor` attacks the encoding before the mathematics, and `math-strategist` refuses to plan a proof for a statement whose fidelity is unresolved — a complete, elegant plan for the wrong statement is worse than no plan, because its polish is what lets the misspecification survive review.
...
Performs breadth-first proof dependency analysis on Lean 4 formalizations. Constructs complete dependency DAGs by recursively expanding until every leaf is classified as `mathlib`, `assumed`, `novel`, or `infrastructure`. Assigns novelty levels (0-5) along five axes (conceptual, theorem, formalization, structural, methodological). Documents axiom boundaries with precise Lean statements and source citations. Generates frontier YAML files and provenance markdown with dependency summaries and a **Conventions** table naming, at each site, the convention chosen and the rival it was chosen over — the documented home the validator's Phase 0.5 convention register checks against. Emits `design_flags` (redundant renames, `def`-not-`abbrev`, trivial aliases, duplicate objects) that feed the library-design auditor.
